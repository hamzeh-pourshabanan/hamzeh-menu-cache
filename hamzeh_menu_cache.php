<?php
/*
Plugin Name: Hamzeh Menu Cache
Plugin URI:
Description: A plugin to cache WordPress menus using the Transients API
Version: 1.0
Author: Hamzeh Pourshabanan
Author URI: https://nikootech.ir
*/
/**
 * hamzeh_menu_cache
 */
...
class hamzeh_menu_cache{
    ...
    function pre_wp_nav_menu($nav_menu, $args){
        ...
        $in_cache = $this->get_menu_transient($args->theme_location);
        ...
        if (isset($in_cache['data']) && isset($last_updated) &&  $last_updated < $in_cache['time'] ){
            $nav_menu = $in_cache['data'].'<!-- From menu cache in '.number_format( microtime(true) - $this->timer, 5 ).' seconds -->';
            return $nav_menu ;
        }
        return $nav_menu;
    }
    ...
    function wp_nav_menu( $nav, $args ) {
        ...
        $key = $this->get_menu_key($args->theme_location);
...
add_action( 'plugins_loaded', 'Hamzeh_Menu_cache_init' );
function Hamzeh_Menu_cache_init() {
    $GLOBALS['wp_menu_cache'] = new hamzeh_menu_cache();
}
